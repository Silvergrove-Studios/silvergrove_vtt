...
| `hm.map.light_at(scene, p)` | `{level="bright" \| "dim" \| "dark", sources, ambient}` — from the scene's own light (`ambient`: `daylight` is bright everywhere, `dim` dim), raised by the lights that reach the point |
