...
**Status 2026-09-21: C1–C9 built** (Hexmap `acbc8f0` … `9f3555b`; then
staging and the filtered creature search in the Maps pane); C9 is
...
| **C5 Prepared encounters** | The Maps pane: the campaign's maps; an encounter builder (map, level, creatures from the compendium with count and cell, hidden, notes; the ruleset's budget when it offers one; the search filters on the facets the ruleset's entry action names — type and CR for 5e); *Launch* → a scene is added, creatures spawned and placed, the scene shown, optionally initiative rolled; *Stage* → the same, but the players' scene stays and only the Table looks at the new one, until *Go*; *Return* → back to the previous scene, encounter-local actors removed, the encounter marked played. | `table.gd`: launch places the creatures where the builder said; return leaves the PCs' wounds and takes the goblins; the 5e journey runs a prepared fight |
