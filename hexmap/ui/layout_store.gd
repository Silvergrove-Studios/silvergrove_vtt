...
const TABLE_PANELS := ["Scenes", "Tokens", "Canvas", "Inspector", "Turns", "Rules", "Compendium", "Players", "Campaign", "Party", "NPCs"]
...
static func table_layout() -> DockableLayout:
	...
	var right := _vsplit(_tabs(["Party", "NPCs", "Campaign"]), _vsplit(_tabs(["Inspector", "Turns", "Rules", "Compendium"]), _leaf("Players"), 0.7), 0.55)
	var inner := DockableLayoutSplit.new()
	inner.direction = DockableLayoutSplit.Direction.HORIZONTAL
	inner.percent = 0.76
