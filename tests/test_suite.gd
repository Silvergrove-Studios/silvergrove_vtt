...
extends RefCounted
## The unit tests, as one object any runner can drive: tests/run_tests.gd
## (`./run.sh test`, headless) and the app's own self-test mode, which runs
## the same suite inside a build on a phone, a simulator or an exported
## desktop app (`--selftest`, or a `user://selftest` marker file). Each
## test_* method asserts with check(); run_all() reports through `say`.
...
## Run every test_* method (or those whose names contain `filter`).
## Tests may await frames, so this is a coroutine.
