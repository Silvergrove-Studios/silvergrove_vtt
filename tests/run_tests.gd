...
func test_encounter_document() -> void:
	...
	check(thin.initiative.round == 1 and thin.players == [], "initiative and players default")
...
func test_encounter_events_roundtrip() -> void:
	...
	var events := [
		...
		{"t": "initiative.set", "changes": {"order": [hero, gob], "running": true, "turn": 1, "round": 3}},
...
func test_encounter_commands() -> void:
	...
	# Initiative wraps rounds both ways.
	c.set_initiative_order([hero, gob, orc.id])
	var ini := st.encounter.initiative
	check(ini.order.size() == 3 and ini.turn == 0 and ini.round == 1 and ini.running, "set_initiative_order")
	c.next_turn(); c.next_turn(); c.next_turn()
	check(ini.turn == 0 and ini.round == 2, "next_turn wraps to a new round")
	c.previous_turn()
	check(ini.turn == 2 and ini.round == 1, "previous_turn wraps back")
	c.previous_turn(); c.previous_turn()
	check(c.previous_turn() != "" and ini.turn == 0 and ini.round == 1, "cannot go before the start")
	c.stop_initiative()
	check(not ini.running, "stop_initiative")
	c.set_initiative_order([])
...
func test_example_encounter() -> void:
	...
	check(e.initiative.order.size() == 6 and not e.initiative.running, "initiative set up, not running")
	for tid in e.initiative.order:
