...
func test_player_join_flow() -> void:
	...
	check(not win._web_button.visible, "no web side heard: no browser button")
	# a table with a web side: its players' page, in the browser (the shell gives the opener)
	win.open_url = func(_u: String) -> void: pass
	win.browser.heard({"name": "Chapel", "port": 47777, "web": 47780}, "10.5.91.189")
	win._refresh_tables()
	check(win._web_button.visible and win._web_button.text.contains("Chapel"), "or play it in the browser: %s" % win._web_button.text)
	win._web_button.pressed.emit()
	check(win.web_opened == "http://10.5.91.189:47780/", "its page: %s" % win.web_opened)
