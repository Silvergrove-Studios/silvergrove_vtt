...
func test_plugin_manifests() -> void:
	...
	check(found.size() >= 3 and str(found[0].id) == "sample.degrees" and str(found[1].id) == "sample.focus" and str(found[2].id) == "sample.ordered", "discover() finds the sample plugins, sorted: %s" % [found.map(func(m): return m.id)])
