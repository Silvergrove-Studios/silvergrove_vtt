...
func test_projection_audience() -> void:
	...
	check(ana.status[0].data.mine == ["a_ana"], "and which actors are hers (a status view offers a new character to a player without one): %s" % [ana.status[0].data.get("mine")])
